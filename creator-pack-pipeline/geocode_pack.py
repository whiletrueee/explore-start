...
META = {
  'empty.japan':       {'name': 'empty.japan',       'tag': 'Japan secret spots',     'color': '#7A4FA3', 'destination': 'Japan',   'center': [37.0, 137.5], 'zoom': 5, 'cityZoom': 12},
  'exploringlondon':   {'name': 'exploringlondon',   'tag': 'London & the UK',         'color': '#2D6A8E', 'destination': 'UK',      'center': [53.0, -1.5],  'zoom': 6, 'cityZoom': 12},
  'thefloridaqueenie': {'name': 'thefloridaqueenie', 'tag': 'Gulf-Coast Florida',      'color': '#F2542D', 'destination': 'Florida', 'center': [27.8, -82.4], 'zoom': 7, 'cityZoom': 12},
  'raimeetravel':      {'name': 'raimeetravel',      'tag': 'Travel hacks + Japan',    'color': '#C9962B', 'destination': 'Japan',   'center': [37.0, 137.5], 'zoom': 5, 'cityZoom': 12},
}
m = META[CREATOR]
...
def resolve(it):
    ...
    if loc:
        ...
        if pt:
            return pt, "exact"
    for q in [it.get("city"), it.get("region")]:
        if q:
            pt = geocode(q + ", Japan")
            if pt:
                return jitter(pt, it["id"]), "approx"
    return None, None
...
for o in out_items:
    if o.get("lat") is not None:
        bycity[o.get("city") or "Around Japan"].append(o)
cityMeta = {}
...
# the creator's itinerary
itins = json.load(open(ROOT + "/itineraries.json"))["itineraries"]
itinerary = next((x for x in itins if x.get("creator") == CREATOR), None)
...
outpath = ROOT + "/prototype/data/pack-" + CREATOR.replace(".", "") + ".json"
