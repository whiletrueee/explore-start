...
Pipeline (per creator):
  ...
  + "hack" family : plan-category gems -> "<dest> on a budget"
...
# ---------------------------------------------------------------- thresholds
FLOOR        = {"hybrid": 4, "type": 4, "theme": 6, "from": 4, "hack": 4}
...
AXIS_WEIGHT = {"hybrid": 4.0, "type": 3.0, "from": 2.5, "hack": 2.5, "theme": 2.0}
...
def make_list(creator, members, axis_type, place, tightness, *, theme=None,
    ...
    place_disp = place or "Travel hacks"
...
def noun_for(axis_type, theme, category, subcategory):
    ...
    if axis_type == "hack": return "budget"
    return axis_type
...
def axis_label(axis_type, theme, category, subcategory):
    ...
    if axis_type == "hack":   return "plan/hack"
    return axis_type
...
def build_headline(axis_type, theme, category, subcategory, place, lens, frame):
    ...
    if axis_type == "hack":
        return f"{place} on a budget" if place else "Money-saving travel hacks"
    return f"{place}"
...
def synthesize(items):
    ...
    for creator, its in by_creator.items():
        ...
        # ---- place pools (exclude roundup-lists, rich itineraries, and plan/hacks —
        #      those are handled by the "from" and "hack" families below)
        ...
        # ---- "hack" family: plan-category gems by destination
        plan = [it for it in its if it.get("category") == "plan"]
        hacks = defaultdict(list)
        for it in plan:
            hacks[it.get("destination") or it.get("region") or creator].append(it)
        for dest, pool in hacks.items():
            res = make_list(creator, dedup_members(pool), "hack", title_dest(dest, creator),
                            "region", frame="in")
            if isinstance(res, dict): creator_lists.append(res)
            elif len(pool) >= 2: attempts_failed[dest].append(res[1])
...
def title_dest(dest, creator):
    if not dest or str(dest).lower() in ("other", "misc", "none", creator):
        return None      # signals a generic "travel hacks" headline
...
def select(cands):
    ...
    # process specific axes first: hybrid > type > from/hack > theme, then by size
